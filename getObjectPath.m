function [path, vel] = getObjectPath(sideLength, gridStep,gridRes, Uinterp, Vinterp, maxDur, dt)
initSize = 2;
...
xIndex(1) = int16(pos(1,1)/gridStep);
yIndex(1) = int16(pos(1,2)/gridStep);
...
while true
    i = i+1;
    dxdt(i) = Uinterp(pos(i-1,:));
    dydt(i) = Vinterp(pos(i-1,:));

    pos(i,1) = pos(i-1,1) + dxdt(i)*dt;
    pos(i,2) = pos(i-1,2) + dydt(i)*dt;

    xIndex(i) = int32(pos(i,1)/gridStep)+1;
    yIndex(i) = int32(pos(i,2)/gridStep)+1;
    ...
    if i*dt>maxDur
        disp('Max search duration reached.')
        break
    end
